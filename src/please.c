...
InstructionInfo instructions[] = {
    ...
    // brr is handled specially below.
    {"brnz",   0xb, "R2", 0},
    {"call",   0xc, "U", 0},
    {"return", 0xd, "N", 0},
    {"brgt",   0xe, "R", 0},
...
// Fields: [opcode (5)][rd (5)][rs (5)][rt (5)][immediate (12)] (unused fields are zeros).
char* assemble_instruction(const char *line) {
    ...
    if (strcasecmp(mnemonic, "brr") == 0) {
        ...
        // If the operand starts with 'r', treat as brr rd.
        if (tokens[1][0] == 'r' || tokens[1][0] == 'R') {
            // Opcode for brr rd is 0x8.
            int opcode = 0x8;
            char opcode_bin[6];
            ...
            // Otherwise, treat operand as an immediate.
            int opcode = 0xa;
            char opcode_bin[6];
            int_to_bin_string(opcode, 5, opcode_bin);
            int imm = atoi(tokens[1]);
            char imm_bin[13];
            // For brr immediate, use signed conversion.
            immediate_to_bin_string(imm, 12, 1, imm_bin);
            // Bits 5-19 are zeros.
            char zeros[16];
            memset(zeros, '0', 15);
            zeros[15] = '\0';
            sprintf(result, "%s%s%s", opcode_bin, zeros, imm_bin);
            return result;
        }
    }
    
    // Special handling for "mov" (all move instructions use the mnemonic "mov")
