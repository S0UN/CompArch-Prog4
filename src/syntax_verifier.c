...
bool validate_instruction(const char *line) {
    ...
    // Branching and Control Flow: brr, br, call, return
        ...
        if (operandCount != 1)
            error("br requires one operand (offset)");
        if (!isValidImmediate(operands[0]) && !isLabelSyntax(operands[0]))
            error("br: operand must be an immediate or a label");
    }
    ...
    // Memory Operations: ld, mov
        ...
        if (operands[0][0] == '(') {  // register-to-memory form
            char reg[10];
            int L;
        ...
        else if (operands[1][0] == '(') {  // memory-to-register form
            char reg[10];
            int L;
